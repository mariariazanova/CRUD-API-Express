...
const checkPath = (pathname: string, etalon: string): boolean => {
  ...
  if (normalizedEtalon.includes(':')) {
    ...
    for (let i = 0; i < etalonParts.length; i += 1) {
      if (
        etalonParts[i] !== pathnameParts[i] &&
        !etalonParts[i].startsWith(':')
      ) {
        return false;
      }
    }
  ...
  const scenarios: ScenariosList = {
    scenarios: {
      scenario1: {
        name: 'get all users',
        appliesTo: () =>
          checkRestMethod(method, RestMethod.Get) &&
          checkPath(pathname, USERS_URL),
      ...
      scenario2: {
        ...
        appliesTo: () =>
          checkRestMethod(method, RestMethod.Get) &&
          checkPath(pathname, USERS_ID_URL),
        execute: () => getUserById(res, pathname),
      },
      scenario3: {
        name: 'create user',
        appliesTo: () =>
          checkRestMethod(method, RestMethod.Post) &&
          checkPath(pathname, USERS_URL),
      ...
      scenario4: {
        name: 'update user',
        appliesTo: () =>
          checkRestMethod(method, RestMethod.Put) &&
          checkPath(pathname, USERS_ID_URL),
      ...
      scenario5: {
        name: 'delete user',
        appliesTo: () =>
          checkRestMethod(method, RestMethod.Delete) &&
          checkPath(pathname, USERS_ID_URL),
    ...
    lastScenario: {
      ...
      appliesTo: () => true,
      execute: () =>
        sendJsonResponse(res, undefined, 404, 'Endpoint is invalid'),
    },
  };
